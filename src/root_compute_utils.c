...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025-09-20 15:53:17 by tcoeffet          #+#    #+#             */
/*   Updated: 2025-09-20 15:53:17 by tcoeffet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	before_last_case(t_quartic *q)
{
	q->sqrtbq = sqrt(q->bq);
...
void	find_solution_to_resolvent_cubic(t_quartic *q)
{
	...
	else if (q->cr2 == q->cq3)
	{
		q->sqrtbq = sqrt(q->bq);
		if (q->br > 0)
			set_d3(q->u, -2 * q->sqrtbq - q->rc / 3, q->sqrtbq - q->rc / 3,
				q->sqrtbq - q->rc / 3);
		else
			set_d3(q->u, -q->sqrtbq - q->rc / 3, -q->sqrtbq - q->rc / 3, 2
				* q->sqrtbq - q->rc / 3);
	}
	else if (q->cr2 < q->cq3)
		before_last_case(q);
