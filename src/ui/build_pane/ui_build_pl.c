...
int	build_pane_pl(void *rt, t_uipane *current)
{
	...
	if (add_btn(setb(&o->color.r, -step, pt(106, 92), BTNSZ), current) ||
		...
		add_btn(setb(&o->shine, -STEP_SH, pt(106, 362), BTNSZ), current) ||
		// add_btn(setb(&o->reflect, -STEP_RF, pt(106, 392}, BTNSZ), current) ||
		...
		add_btn(setb(&o->shine, STEP_SH, pt(136, 362), BTNSZ), current))// ||
		// add_btn(setb(&o->reflect, STEP_RF, pt(136, 392}, BTNSZ), current))
		return (1);
	return (0);
}