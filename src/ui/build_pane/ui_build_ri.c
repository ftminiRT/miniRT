...
int	build_pane_ri(void *rt, t_uipane *current)
{
	...
	if (add_btn(setb(&o->color.r, -step, pt(106, 92), BTNSZ), current) ||
		...
		add_btn(setb(&o->scal2, -step, pt(106, 392), BTNSZ), current) ||
		// add_btn(setb(&o->scal3, -step, pt(106, 422), BTNSZ), current) ||
		add_btn(setb(&o->shine, -STEP_SH, pt(106, 452), BTNSZ), current) ||
		// add_btn(setb(&o->reflect, -STEP_RF, pt(106, 482}, BTNSZ), current) ||
		...
		add_btn(setb(&o->scal2, step, pt(136, 392), BTNSZ), current) ||
		// add_btn(setb(&o->scal3, step, pt(136, 422), BTNSZ), current) ||
		add_btn(setb(&o->shine, STEP_SH, pt(136, 452), BTNSZ), current))// ||
		// add_btn(setb(&o->reflect, STEP_RF, pt(136, 482}, BTNSZ), current))
		return (1);
	return (0);
}