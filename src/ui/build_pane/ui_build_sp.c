...
int	build_pane_sp(void *rt, t_uipane *current)
{
	...
	if (add_btn(setb(&o->color.r, -step, pt(106, 92), BTNSZ), current) ||
		...
		add_btn(setb(&o->scal, -step, pt(106, 272), BTNSZ), current) ||
		add_btn(setb(&o->shine, -STEP_SH, pt(106, 302), BTNSZ), current) ||
		...
		add_btn(setb(&o->scal, step, pt(136, 272), BTNSZ), current) ||
		add_btn(setb(&o->shine, STEP_SH, pt(136, 302), BTNSZ), current))// ||
		// add_btn(setb(&o->reflect, -STEP_RF, pt(106, 332}, BTNSZ), current) ||
		// add_btn(setb(&o->reflect, STEP_RF, pt(136, 332}, BTNSZ), current))
		return (1);
	return (0);
}